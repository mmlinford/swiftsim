...
runner_iact_nonsym_stars_feedback(
  ...
  /* Get a few variables */
  /* const float e_sn = stars_properties->feedback.energy_per_supernovae; */
  const float m_ej = stars_properties->feedback.mass_ejected;
  /* const float e_sn_normalized = e_sn * m_ej / si->density.rho_gas; */
  /* const float u_old = hydro_get_physical_internal_energy(pj, xp, cosmo); */

  /* Mass received */
  const float dm = m_ej * wi;

  /* Energy received */
  /* const float du = m_ej * e_sn_normalized * wi / (mj + dm); */
